...
#'       curTree = extract.clade(tree, curNodeIdx)
...
#'   #curMeta = as_tibble(curMeta, rownames = "ID")
...
#' 
#' 
#' # Return a tree dataframe for mapping 
...
#'     d = Descendants(x = tree, node = cladeList$nodeID[i], type = "all")
#'     
#'     # 5B. Iterate through each of the descendants
#'     for(j in 1:length(d)){
#'       if(d[j] %in% nodeIDList){ # 5B.1 The desendant is in the nodeList
#'         # Remove desendant 
...
#' }
#' 
#' # Now plot again but without tips that don't have ancestors of significant nodes. And we need to label phylums
...
#'   
#'   # Make data matrix 
#'   a = Ancestors(x = tree, node = 1:N, type = "all")
#'   
...
#' # Makes NB 
#' # Tests GLM with base variables + microbial feature
#' # Returns a dataframe with covariate tested, estimate, microbial feature, p value, and q value
...
#'       d[count, "clade"] = curMicrobes[j]
#'       #print(curFormula)
#'       #curModel = glm.nb(formula = curFormula, data = curData, na.action = "na.omit", link = "log", control=glm.control(maxit=100))# try glm.nb first
#'       
#'       
...
#'         }else{
#'           #curData[[curCovs]] = curData[[curCovs]] + 0.01
#'           curData[[curMicrobes[j]]] = curData[[curMicrobes[j]]] + 0.00001
#'           curModel = tryGLMNBModel(curFormula = curFormula, curData = curData)
#'           #curModel = tryGLMNBModelZeroFunc(curFormula = curFormula, curData = curData)
#'           if(is.na(curModel)){
...
#' }
#' 
#' cladeLabels = function(dataFrame, idxs, nodeName, color, fontsize){
#'   
#'   cladelabels = ""
